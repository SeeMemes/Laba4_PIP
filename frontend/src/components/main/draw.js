...
function draw(r,context){
    ...
    context.beginPath();
    context.moveTo(x, y);
    context.arc(x, y, step, 0, Math.PI/2);
    context.fill();
...
function clickPoint(key, event,context,r, validateNumber, messageText, setResults){
    ...
    if (!validateNumber(x, -3, 5) || !validateNumber(y, -3, 5) ||!validateNumber(r, 0.01, 5)){
        ...
        fetch("/api/points/add", {
            ...
            body: JSON.stringify({key: store.getState().token, x: x, y:y, r: r})
        }).then(response => response.text().then(text => {
            if (response.ok) {
                setResults(JSON.parse(text).data)
                console.log("Вывод от клика по изображению: ", JSON.parse(text).data)
            }
